...
#  Using the encrypted ASCII codes below, and the knowledge that the plain text must contain
#  common English words, decrypt the message and find the sum of the ASCII values in the original text.

def decrypt_chars(chars, key)
  chars.each.with_index.reduce('') do |str, (char, idx)|
    str + (char.ord ^ key[idx % key.size].ord).chr
  end
end

encrypted_chars = DATA.first.strip.split(',').map(&:to_i)
encryption_key_length = 3

possible_key = ('aaa'..'zzz').detect do |key|
  encryption_key = key.split(/(?=.)/).map { |char| char.ord }
  decrypted = decrypt_chars(encrypted_chars, key)

  decrypted.match(/ the /) && decrypted.match(/ and /)
end

p decrypt_chars(encrypted_chars, possible_key).split(/(?=.)/).reduce(0) { |sum, char| sum + char.ord }
