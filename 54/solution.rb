...
#  hand are compared (see example 4 below); if the highest cards tie then the next highest
...
#
# Below are one-thousand random hands dealt to two players. Each line contains ten cards
#  (separated by a single space): the first five are Player 1's cards and the last five
#  are Player 2's cards. You can assume that all hands are valid (no invalid characters
#  or repeated cards), each player's hand is in no specific order, and in each hand
...
class Hand
  ...
  def initialize(cards_str)
    @cards = cards_str.map { |card_str| Card.new(card_str) }
  end

  def high_card
    cards.max_by { |card| RANKS[card.rank] }
  end

  def sorted
    if four_of_a_kind?
      rank = four_of_a_kind_rank
      sorted_cards = cards.select { |card| card.rank == rank }
      sorted_cards + (cards - sorted_cards)
    elsif full_house?
      rank = three_of_a_kind_rank
      triplet = cards.select { |card| card.rank == rank }
      triplet + two_of_a_kind_cards
    elsif three_of_a_kind?
      rank = three_of_a_kind_rank
      triplet = cards.select { |card| card.rank == rank }
      triplet + sort_by_rank(cards - triplet)
    elsif two_pair?
      pairs = two_of_a_kind_cards
      pairs + (cards - pairs)
    elsif two_of_a_kind?
      pair = two_of_a_kind_cards
      pair + sort_by_rank(cards - pair)
    else
      sort_by_rank(cards)
    end
  end

  def sort_by_rank(cards_to_sort)
    cards_to_sort.sort_by { |card| RANKS[card.rank] }.reverse
  end

  def royal_flush?
    flush? &&
      ['T', 'J', 'Q', 'K', 'A'].all? do |rank|
  ...
  def straight?
    rank_vals = cards.map { |card| RANKS[card.rank] }
    min = rank_vals.min
    max = rank_vals.max
    max - min == 4 && (min..max).all? do |rank|
      cards.any? do |card|
        RANKS[card.rank] == rank
      end
    end
  end
  ...
  def x_of_a_kind?(x)
    by_rank.max_by { |_, cards| cards.size }.last.size == x
  end
  ...
  def two_pair?
    by_rank.select do |rank, cards|
      cards.size == 2
    end.size == 2
  end

  def two_of_a_kind?
    by_rank.detect { |_, cards| cards.size == 2 }
  end

  def four_of_a_kind_rank
    return nil unless four_of_a_kind?

    x_of_a_kind_rank!(4)
  end

  def three_of_a_kind_rank
    return nil unless three_of_a_kind?

    x_of_a_kind_rank!(3)
  end

  def two_of_a_kind_cards
    return nil unless two_of_a_kind?

    ranks = by_rank.select do |_, cards|
      cards.size == 2
    end.map { |rank, _| rank }

    cards
      .select { |card| ranks.include?(card.rank) }
      .sort_by { |card| RANKS[card.rank] }
      .reverse
  end

  def x_of_a_kind_rank!(x)
    by_rank.detect do |rank, cards|
      cards.size == x
    end.first
  end
  ...
  def value
    if royal_flush?
      ...
      8.0 + RANKS[high_card.rank] / 100.0
    elsif four_of_a_kind?
      7.0 + RANKS[four_of_a_kind_rank] / 100.0
    elsif full_house?
      6.0 + RANKS[three_of_a_kind_rank] / 100.0
    elsif flush?
      5.0 + self.class.value_of(sorted)
    elsif straight?
      4.0 + RANKS[high_card.rank] / 100.0
    elsif three_of_a_kind?
      3.0 + RANKS[three_of_a_kind_rank] / 100.0
    elsif two_pair?
      2.0 + self.class.value_of(sorted)
    elsif two_of_a_kind?
      1.0 + self.class.value_of(sorted)
    else
      0.0 + self.class.value_of(sorted)
    end
  end
  ...
  def self.value_of(cards)
    cards.each.with_index.reduce(0) do |sum, (card, idx)|
      # p "----> #{card.rank} / 100 ** #{idx + 1}"
      sum + RANKS[card.rank] / (100 ** (idx + 1)).to_f
    end
  end

end

hands = DATA.map do |row|
  both = row.split(' ')
  [Hand.new(both[0..4]), Hand.new(both[5..9])]
end
...
player1_wins = hands.select do |hand_ary|
  player1 = hand_ary.first
  player2 = hand_ary.last
